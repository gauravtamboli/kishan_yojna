...
@Component({
  selector: 'app-officers-dashboard-supreme',
...
export class OfficersDashboardSupremePage implements OnInit {
  ...
  async loadCircles() {
    this.showDialog("वृत्त लोड हो रहे हैं...");
    try {
      this.apiService.getCircles().subscribe(
        async (response) => {
          if (response.response.code === 200) {
            this.listOfCircles = response.data || [];
            if (this.listOfCircles.length > 0) {
              // Auto-select first circle if none selected
              if (!this.selectedCircleId && this.listOfCircles.length > 0) {
                this.selectedCircleId = this.listOfCircles[0].id.toString();
                this.selectedCircleName = this.listOfCircles[0].name;
            ...
            await this.dismissDialog();
            this.longToast(response.response.msg || 'वृत्त लोड करने में त्रुटि');
          }
        },
        async (error) => {
          await this.dismissDialog();
          this.shortToast('वृत्त लोड करने में त्रुटि');
        }
      );
    } catch (error) {
      await this.dismissDialog();
      this.shortToast('त्रुटि हुई');
    }
  }
  ...
  onCircleChange(event: any) {
    ...
    if (selectedValue === 'all') {
      ...
      this.selectedCircleName = 'सभी वृत्त';
      this.currentPage = 1;
      // Clear previous data to avoid showing stale list
      this.listOfAwedan = [];
      this.filteredAwedans = [];
      this.getDashboardDataFromServer();
    } else {
      // Handle specific circle selection
      const selectedCircle = this.listOfCircles.find(c => c.id.toString() === selectedValue);
      if (selectedCircle) {
        this.selectedCircleId = selectedCircle.id.toString();
        this.selectedCircleName = selectedCircle.name;
        this.currentPage = 1;
        // Clear previous data to avoid showing stale list
        this.listOfAwedan = [];
        this.filteredAwedans = [];
  ...
  getListOfAwedanAfterClickOnBoxes(whichBoxClickeddd: number, page: number = 1) {
    //debugger;
    console.log('selectedCircleId:', this.selectedCircleId);
    if (!this.selectedCircleId) {
      this.shortToast('कृपया वृत्त चुनें');
      return;
    }
